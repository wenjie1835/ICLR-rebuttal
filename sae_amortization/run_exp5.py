...
def ista_lasso(X: np.ndarray, D: np.ndarray, lmbd: float, steps: int = 200, step_size: Optional[float] = None, z0: Optional[np.ndarray] = None) -> np.ndarray:
    """
    Solve min_z 0.5||x - zD^T||^2 + lambda||z||_1 (row-wise), D: [d_in, n_lat]
    Returns Z of shape [n, n_lat]. If z0 is provided, use it as initial (semi-amortized).
    """
    n, d = X.shape
    d_in, n_lat = D.shape
    assert d == d_in
    ...
    if z0 is None:
        ...
        if Z.shape != (n, n_lat):
            raise ValueError(f"z0 shape {Z.shape} != ({n}, {n_lat})")
    Dt = D.T
...
def main():
    ...
    # eval_modes: amortized | semi_amortized | non_amortized (for rebuttal: batch_topk / matryoshka_batch_topk three-mode comparison)
    eval_modes = list(cfg.get("eval_modes", ["amortized"]))
    if not eval_modes:
        eval_modes = ["amortized"]
    semi_steps = int(cfg.get("semi_steps", 30))
    non_steps = int(cfg.get("non_steps", 200))
    target_dense_semi_non = float(cfg.get("target_dense_semi_non", 0.1))
    ...
    for _, row in df.iterrows():
        ...
        # Calibrate lambda once for semi/non-amortized (shared across modes for fair comparison)
        used_lambda_semi_non = None
        if "semi_amortized" in eval_modes or "non_amortized" in eval_modes:
            n_cal = min(max(2000, n_use // 5), n_use)
            try:
                used_lambda_semi_non = calibrate_lambda(
                    X_sub[:n_cal], D, target_dense=target_dense_semi_non, steps=min(100, non_steps // 2)
                )
            except Exception:
                used_lambda_semi_non = 1e-3

        for mode in eval_modes:
            if mode == "amortized":
                Z = encode_amortized(X_sub, W_enc, b_enc, variant, target_dense=ag_target_dense, guessed_k=guessed_k)
            elif mode == "semi_amortized":
                Z_amort = encode_amortized(X_sub, W_enc, b_enc, variant, target_dense=ag_target_dense, guessed_k=guessed_k)
                Z = ista_lasso(X_sub, D, used_lambda_semi_non or 1e-3, steps=semi_steps, z0=Z_amort)
            elif mode == "non_amortized":
                Z = ista_lasso(X_sub, D, used_lambda_semi_non or 1e-3, steps=non_steps, z0=None)
            else:
                continue
            ...
            if ag_enabled and mode == "amortized":
                ...
                used_lambda = float(ag_lambda) if ag_lambda is not None else (calibrate_lambda(X_gap[: min(max(2000, n_gap // 5), n_gap)], D, target_dense=ag_target_dense, steps=ag_steps) if ag_calib else 1e-3)
                try:
                ...
                m_row["amort_gap_mean"] = float("nan")
                m_row["amort_gap_median"] = float("nan")
                m_row["amort_lambda"] = float("nan")
